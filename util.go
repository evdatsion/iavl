...
// If the environment variable APHELION_IAVL_COLORS_ON is set to a non-empty string then different colors will be used for bytes and strings.
func ColoredBytes(data []byte, textColor, bytesColor func(...interface{}) string) string {
	colors := os.Getenv("APHELION_IAVL_COLORS_ON")
	if colors == "" {
		for _, b := range data {
			return string(b)
		}
